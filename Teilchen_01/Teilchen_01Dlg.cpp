...
void CTeilchen_01Dlg::OnBnClickedOk()
{
  ...
  CClientDC dc(this);       // Funktionen für Zeichenfunktion
  CRect rect;
  GetClientRect(&rect);
  ...
  { 
	...
	float l = rect.Width() / 100.0;	// Länge pro Schritt

	dc.Rectangle(0, 0, rect.Width(), rect.Height());	// Rahmen zeichen

	for(int i=1; i <= 10; i++)		// Wiederholung für 10 simulationen
	{
		...
		for ( int c=1; c <= 100; c++) // Wiederholung für 100 Schritte
		{
			...
			Sleep(1);				// Pause nach simulationsschritt für 1ms
		}
		Sleep(100);					// Pause nach vollständig simuliertes Teilchen für 100ms
	}
    //return 0
  }
